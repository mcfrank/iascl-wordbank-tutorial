...
```{r}
# your code here:
admins_ces_ws <- get_administration_data(language = "Czech",
                                         form = "WS",
                                         include_demographic_info = TRUE)
admins_ces_ws %>% 
  count(age)
...
summary_eng_ws <- admins_eng_ws %>% 
  ...
  summarise(mean_production = mean(production),
            n = n()) %>% 
  filter(!is.na(sex))
ggplot(summary_eng_ws,
       aes(x = age, y = mean_production, col = sex)) +
...
```{r}
# your code here:
summary_ces_ws <- admins_ces_ws %>% 
  group_by(age, sex) %>% 
  summarise(mean_production = mean(production),
            n = n()) %>% 
  filter(!is.na(sex))
ggplot(summary_ces_ws,
       aes(x = age, y = mean_production, col = sex)) +
  geom_point(aes(size = n)) +
  geom_smooth() +
  labs(x = "Age (months)",
...
```{r}
# your code here:
ball_because_items <- items_eng_ws |> 
  filter(item_definition %in% c("ball", "because"))

ball_because_data <- get_instrument_data(language = "English (American)", 
                                         form = "WS", 
                                         items = ball_because_items$item_id, 
                                         item_info = TRUE)

ball_because_means <- ball_because_data |>
  left_join(admins_eng_ws) |>
  filter(!is.na(sex)) |>
  group_by(age, sex, item_definition) |>
  summarise(produces = mean(produces, na.rm=TRUE))

ggplot(ball_because_means, aes(x = age, y = produces, col = item_definition)) +
  geom_point() + 
  geom_smooth() +
  facet_wrap(~sex) +
  labs(x = "Age (months)",
...
ggplot(admins_eng_ws, 
  ...
  geom_jitter(alpha = .1, size = .5, width = 0, height = 0) + 
  geom_smooth() +
